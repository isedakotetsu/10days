...
void Player::Initialize(Model* model) {
	...
	// X=0で左右中央
	// Y=0で地面に配置
	worldTransform_.translation_ = {0.0f, 0.635770f, 0.0f};

	// Y軸方向に180度回転
	worldTransform_.rotation_ = {0.0f, 3.141592f, 0.0f};
...
void Player::Update(const block* blockObject) {
	...
	if (blockObject != nullptr) {
		...
		for (const auto& blockWorldTransform : blockObject->GetBlocks()) {
			ResolveBlockCollision(*blockWorldTransform, blockObject->GetBlockWidth(), blockObject->GetBlockHeight(), previousPosition);
		}

		// 現在左右に動いているブロックとの当たり判定
		if (!blockObject->IsFalling()) {
			ResolveBlockCollision(blockObject->GetMovingBlock(), blockObject->GetBlockWidth(), blockObject->GetBlockHeight(), previousPosition);
		}
	}
...
void Player::ResolveBlockCollision(const WorldTransform& blockWorldTransform, float blockWidth, float blockHeight, const Vector3& previousPosition) {
	...
	const float blockLeft = blockWorldTransform.translation_.x - blockHalfWidth;
	const float blockRight = blockWorldTransform.translation_.x + blockHalfWidth;
	const float blockBottom = blockWorldTransform.translation_.y - blockHalfHeight;
	const float blockTop = blockWorldTransform.translation_.y + blockHalfHeight;

	float playerLeft = worldTransform_.translation_.x - kPlayerHalfWidth;
	float playerRight = worldTransform_.translation_.x + kPlayerHalfWidth;
	float playerBottom = worldTransform_.translation_.y - 0.635770f;
	float playerTop = playerBottom + kPlayerHeight;

	const bool overlapsX = playerRight > blockLeft && playerLeft < blockRight;

	// ブロックの上にちょうど立っている状態も接地として扱う
	constexpr float kContactTolerance = 0.05f;
	if (overlapsX && playerBottom >= blockTop - kContactTolerance && playerBottom <= blockTop + kContactTolerance && velocityY_ <= 0.0f) {
	...
	const bool overlapsY = playerTop > blockBottom && playerBottom < blockTop;
	...
	if (previousBottom >= blockTop - kContactTolerance && velocityY_ <= 0.0f) {
		worldTransform_.translation_.y = blockTop + 0.635770f;
		velocityY_ = 0.0f;
		isOnGround_ = true;
		return;
